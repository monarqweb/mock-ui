...
async function renderDemo() {
  ...
  try {
    ...
    const hasExports =
      /export\s+(?:default\s+)?(?:function|const|class|\w+)/.test(
        codeWithoutImports
      )
    ...
    for (const path of uniquePaths) {
      ...
      if (path.startsWith("@/components/blocks/")) {
        ...
        if (componentName.includes("/")) {
          importedModules[path] = await import(`@/components/blocks`)
        } else {
          importedModules[path] = await import(
    ...
    if (hasExports) {
      ...
      // 4. first named export ending with "Demo"
        ...
        const demoExports = Object.keys(exportsObj).filter((k) =>
          k.endsWith("Demo")
        )
      ...
      if (!component) {
        const exportKeys = Object.keys(exportsObj).filter(
          (k) => k !== "default"
        )
      ...
      if (!component) {
        const allExports = Object.keys(exportsObj).filter(
          (k) => k !== "default"
        )
        throw new Error(
      ...
      const evalCode = new Function("React", ...importKeys, transformedCode)
